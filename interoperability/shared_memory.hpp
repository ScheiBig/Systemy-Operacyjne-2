...
namespace i_op
{
    ...
    public:
         ...
         */
        inline shared_memory(char const* _name, size_t _n_elem) noexcept(false);
         ...
         */
        inline void unmap() noexcept(false);
        /**
...
inline i_op::shared_memory<T, _cr_new>::shared_memory(char const* _name, size_t _n_elem)
{
#ifdef OS_WIN32
...
template<typename T, bool _cr_new>
inline i_op::shared_memory<T, _cr_new>::~shared_memory()
{
...
template<typename T, bool _cr_new>
inline void i_op::shared_memory<T, _cr_new>::unmap()
{
#ifdef OS_WIN32
...
template<typename T, bool _cr_new>
inline void i_op::shared_memory<T, _cr_new>::close()
{
#ifdef OS_WIN32
...
#ifdef OS_LINUX
    int e[3]{ 0 };
    if (munmap(this->__ptr, this->__len * sizeof(T)) != 0) { e[0] = errno; }
    if (shm_unlink(this->__name) != 0) { e[1] = errno; }
    if (::close(this->__fl_dtr) != 0) { e[2] = errno; }
    this->__fl_dtr = -1;
#endif
    this->__name = nullptr;
#ifdef OS_LINUX
